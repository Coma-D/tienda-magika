        ...
        <div className="p-6">
          <div className="space-y-4">
            {items.map((item) => (
              <div key={item.card.id} className="flex items-center space-x-4 p-4 border rounded-lg">
                <img src={item.card.image} alt={item.card.name} className="w-16 h-16 object-cover rounded" />
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900">{item.card.name}</h3>
                  <p className="text-sm text-gray-600">{item.card.set}</p>
                  <p className="text-lg font-bold text-green-600">{formatCLP(item.card.price)}</p>
                </div>
                <div className="flex items-center space-x-2">
                  <button onClick={() => updateQuantity(item.card.id, item.quantity - 1)} className="p-1 hover:bg-gray-100 rounded">
                ...
                <div className="text-right min-w-[100px]">
                  <p className="font-semibold text-gray-900">{formatCLP(item.card.price * item.quantity)}</p>
                </div>
                <button onClick={() => removeFromCart(item.card.id)} className="p-2 text-red-600 hover:bg-red-50 rounded">
          ...
          <div className="mt-6 pt-6 border-t">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-2xl font-bold text-green-600">{formatCLP(total)}</span>
            </div>
            <Button onClick={onCheckout} className="w-full" size="lg">Finalizar compra</Button>
          </div>
        </div>
      </div>
    </div>
